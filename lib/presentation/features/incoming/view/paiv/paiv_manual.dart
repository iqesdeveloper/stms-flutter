...
class PaivManual extends StatefulWidget {
  // final Function changeView;

  const PaivManual({Key? key}) : super(key: key); //, required this.changeView

  @override
  _PaivManualState createState() => _PaivManualState();
}

class _PaivManualState extends State<PaivManual> {
  List paivItemListing = [];
  List paivSerialNo = [];
  List getInfoPaiv = [];
  var selectedLoc, selectedInvtry, tracking, infoPaiv;
      ...
      DBPaivItem().getAllPaivItem().then((value) {
        if (value != null) {
          ...
          if (null == itemPaiv) {
            ...
            Navigator.of(context).pushNamed(StmsRoutes.paivItemDetail);
          } else {
