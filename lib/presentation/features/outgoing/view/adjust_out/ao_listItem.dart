...
import 'package:stms/data/api/repositories/api_json/api_out_adjust.dart';
...
class AoListItem extends StatefulWidget {
  const AoListItem({Key? key}) : super(key: key);

  @override
  _AoListItemState createState() => _AoListItemState();
}

class _AoListItemState extends State<AoListItem> {
  // ignore: unused_field
  ...
  getAdjustItem() {
    DBAdjustOutItem().getAllAoItem();
    DBAdjustOutNonItem().getAllAoNonItem();
  }
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        Container(
                          // padding: const EdgeInsets.fromLTRB(5, 0, 5, 0),
                          child: FutureBuilder(
                            future: DBAdjustOutItem().getAllAoItem(),
                            builder: (
                        ...
                        Container(
                          child: FutureBuilder(
                            future: DBAdjustOutNonItem().getAllAoNonItem(),
                            builder: (
                              BuildContext context,
                              AsyncSnapshot snapshot,
                            ) {
  ...
  Future uploadAdjustOUT() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();

    DBAdjustOutItem().getUpload().then((value) {
      aoSerial = value;
    });

    DBAdjustOutNonItem().getUpload().then((value) {
      aoNonTrack = value;
      if (aoSerial != null && aoNonTrack != null) {
        combineUpdated = []
          ..addAll(aoSerial)
          ..addAll(aoNonTrack);
      } else if (aoSerial == null) {
        combineUpdated = aoNonTrack;
      } else {
        combineUpdated = aoSerial;
      }

      AdjustOutService().sendToServer(combineUpdated).then((value) {
        if (value['status'] == true) {
          ...
          Navigator.popUntil(
              context, ModalRoute.withName(StmsRoutes.adjustOut));
  ...
  Future<void> saveData(String barcodeScanRes) async {
    ...
    DBAdjustOutItem().getAllAoItem().then((value) {
      if (value != null) {
        ...
        var itemAo = aoItemListing.firstWhereOrNull(
